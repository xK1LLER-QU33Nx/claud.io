...
export function useDeferredHookMessages(
  ...
  return useCallback(async () => {
    ...
    // Safety timeout to avoid hanging the REPL forever
    const timeoutPromise = new Promise<HookResultMessage[]>(resolve => {
      setTimeout(() => resolve([]), 2000);
    });

    const msgs = await Promise.race([promise, timeoutPromise])
