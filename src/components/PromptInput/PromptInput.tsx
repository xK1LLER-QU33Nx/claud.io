...
function PromptInput({
  ...
  const onSubmit = useCallback(async (inputParam: string, isSubmittingSlashCommand = false) => {
    try {
      fs.appendFileSync('gemini_debug.log', `[PromptInput] onSubmit called. input="${inputParam.substring(0, 20)}" isSlash=${isSubmittingSlashCommand}\n`);
    } catch (e) {}
    process.stderr.write(`[DEBUG:PromptInput] onSubmit called: "${inputParam.substring(0, 50)}"\n`);
    ...
    if (state.footerSelection && footerItems.includes(state.footerSelection)) {
      try {
        fs.appendFileSync('gemini_debug.log', `[PromptInput] onSubmit early return: footerSelection active (${state.footerSelection})\n`);
      } catch (e) {}
      return;
    }
    ...
    if (suggestionsState.suggestions.length > 0 && !isSubmittingSlashCommand && !hasDirectorySuggestions) {
      try {
        fs.appendFileSync('gemini_debug.log', `[PromptInput] onSubmit early return: suggestions active (length=${suggestionsState.suggestions.length})\n`);
      } catch (e) {}
    ...
    try {
      fs.appendFileSync('gemini_debug.log', `[PromptInput] calling onSubmitProp. input="${inputParam.substring(0, 20)}"\n`);
    } catch (e) {}
  ...
  const baseProps: BaseTextInputProps = {
    multiline: true,
    onSubmit: (originalValue: string) => {
      try {
        fs.appendFileSync('gemini_debug.log', `[useTextInput] handleEnter calling onSubmit. originalValue="${originalValue.substring(0, 20)}"\n`);
      } catch (e) {}
      onSubmit(originalValue);
      try {
        fs.appendFileSync('gemini_debug.log', `[useTextInput] handleEnter onSubmit called (returned).\n`);
      } catch (e) {}
    },
    onChange,
